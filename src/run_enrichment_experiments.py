...
def _gen_answer(oai, question: str, context: str):
    ...
    r = oai.chat.completions.create(
        model=config.OPENAI_CHAT_MODEL, temperature=0.0, max_tokens=256,
    ...
    u = r.usage
    return r.choices[0].message.content.strip(), u.prompt_tokens, u.completion_tokens, ms


def run_condition(dataset: str, cond: str, recs: List[Dict], queries: List[Dict],
                  client, embedder, oai, judge, top_k: int, retrieve_k: int,
                  concurrency: int, do_answers: bool) -> Dict[str, Any]:
    ...
    metas = [{"source_id": r["metadata"]["source_id"], "dataset": dataset,
              "condition": cond} for r in recs]
    n_chunks = len(recs)
    avg_tokens = round(sum(common.estimate_tokens(t) for t in texts) / max(n_chunks, 1), 2)
    ...
    for i in range(0, n_chunks, B):
        ...
        collection.add(ids=ids[i:i + B], embeddings=vecs,
    ...
    def worker(q):
        ...
        res = collection.query(query_embeddings=[qv], n_results=retrieve_k,
        ...
        context = "\n\n---\n\n".join(rdocs[:top_k])
        out = {"query_id": q["query_id"], "ranked": ranked,
               "gold": [str(g) for g in q.get("gold_source_ids", [])],
               "emb_ms": emb_ms, "srch_ms": srch_ms,
               "total_ms": emb_ms + srch_ms,
               "gold_answer": q.get("gold_answer", ""), "question": q["text"],
               "answer": "", "ptok": 0, "ctok": 0, "gen_ms": 0.0,
               "faithfulness": None, "answer_relevance": None,
               "citation_accuracy": None, "answer_correctness": None}
        if do_answers:
            try:
    ...
    def dedup(r):
        ...
        for s in r:
            if s not in seen:
                seen.add(s); o.append(s)
        return o
    rm = {k: 0.0 for k in ("Recall@5", "Recall@10", "Precision@5", "MRR", "nDCG@10", "Hit@5")}
    scored = [r for r in results if r["gold"]]
    for r in scored:
        ranked = dedup(r["ranked"]); gold = set(r["gold"])
    ...
    # ---- latency aggregates ----
    embms = [r["emb_ms"] for r in results]; srch = [r["srch_ms"] for r in results]
    tot = [r["total_ms"] for r in results]
    latency = {
        "num_chunks": n_chunks, "avg_tokens_per_chunk": avg_tokens,
    ...
    answer = {"num_answers": 0, "faithfulness": None, "citation_accuracy": None,
              "answer_relevance": None, "exact_match": None, "token_f1": None,
              "answer_correctness": None}
    pin, cout = (sum(r["ptok"] for r in results), sum(r["ctok"] for r in results))
    cost = {"prompt_tokens": pin, "completion_tokens": cout,
            "total_tokens": pin + cout, "estimated_cost_usd": 0.0}
    if do_answers:
        ...
        em_vals = [ea.exact_match(r["answer"], r["gold_answer"]) for r in results if r["gold_answer"]]
        f1_vals = [ea.token_f1(r["answer"], r["gold_answer"]) for r in results if r["gold_answer"]]
        answer = {
    ...
    return {"retrieval": retrieval, "latency": latency, "answer": answer,
            "cost": cost, "per_query": results, "num_queries": len(results)}
...
def main() -> None:
    ...
    ap.add_argument("--distractors", type=int, default=60,
    ...
    for dataset in args.datasets:
        ...
        for cond, recs in conditions.items():
            if not recs:
                print(f"  {dataset}/{cond}: no records, skip"); continue
            r = run_condition(dataset, cond, recs, queries, client, embedder,
                              oai, judge, args.top_k, retrieve_k,
                              args.concurrency, do_answers)
            tag = f"{dataset}/{cond}"
            print(f"  {tag:<48} nDCG@10={r['retrieval']['nDCG@10']:.3f} "
                  f"faith={r['answer']['faithfulness']} "
                  f"idx={r['latency']['total_index_time_seconds']}s "
                  f"${r['cost']['estimated_cost_usd']}")
            ret_rows.append({"dataset": dataset, "method": cond,
                             "num_queries": r["num_queries"], **r["retrieval"]})
