...
import adaptive_questions_100 as A  # noqa: E402
import article_chunk_questions_full as C  # noqa: E402
from embeddings import embedding_signature  # noqa: E402
...
UNICODE_WORD = re.compile(r"[^\W_]+", re.UNICODE)


def tokenize(text: str) -> list[str]:
    return [word.casefold() for word in UNICODE_WORD.findall(text or "")]
    ...
    for qi, query in enumerate(queries):
        ...
        for order in (dense, sparse):
            for rank, chunk_id in enumerate(order, 1):
                score[chunk_id] += 1 / (A.RRF_K + rank)
        ranking = [
            item
            for item, _ in sorted(score.items(), key=lambda pair: (-pair[1], pair[0]))
        ]
...
def combined_fast(
    ...
    for qi, query in enumerate(queries):
        ...
        for order in (dense, sparse):
            for rank, chunk_id in enumerate(order, 1):
                rrf[chunk_id] += 1 / (A.RRF_K + rank)
        ranking = [
            item
            for item, _ in sorted(rrf.items(), key=lambda pair: (-pair[1], pair[0]))
        ]
