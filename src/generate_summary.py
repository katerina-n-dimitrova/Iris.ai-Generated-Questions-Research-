...
BASELINE_REPR = {
    "scifact": "Raw abstract sentence",
    "nfcorpus": "Raw biomedical passage chunk",
    "wikitablequestions": "Linearized table row",
    "chartqa": "Chart OCR/caption text",
...
def main() -> None:
    ...
    L.append(
        "We tested whether different context-enrichment methods improve RAG "
    ...
    for ds in datasets:
        ...
        L.append(
            f"| {ds} | {DATA_TYPE.get(ds, '')} | {BASELINE_REPR.get(ds, '')} | "
    ...
    for ds in datasets:
        ...
        if acand:
            ...
            L.append(
                f"- **Best answer-quality method:** `{abest[0]}` "
                f"(faithfulness {abest[1]:.3f}, baseline {_fmt(base_a.get('faithfulness'))})"
            )
        L.append(
            f"- Baseline → retrieval nDCG@10 {_fmt(base_r.get('nDCG@10'))}, "
        ...
        for m in methods[ds]:
            ...
            if m != "baseline":
                ...
                verdict = (
                    ...
                    if ret_up and not ans_up
                    else "retrieval ↓ / answer ↑"
                    if not ret_up and ans_up
                    else "both ↓"
                )
            L.append(
                f"| {ds} | {m} | {_fmt(rr.get('Recall@5'))} | {_fmt(rr.get('MRR'))} | "
    ...
    for ds in datasets:
        ...
        def best_by(metric_map, base, key, maximize=True):
            ...
            return (
                f"{pick[0]} ({pick[1] - base:+.3f})"
                if base is not None
                else f"{pick[0]} ({pick[1]:.3f})"
            )
        ...
        for m in nb:
            nd, fa = (
                _f(ret[(ds, m)].get("nDCG@10")),
                _f(ans.get((ds, m), {}).get("faithfulness")),
            )
            if nd is None:
                continue
            score = (nd - (base_nd or 0)) + (
                (fa - base_fa) if (fa is not None and base_fa is not None) else 0
            )
    ...
    qa = {
        "Structured scientific text (SciFact)": best_ret("scifact")
        if "scifact" in datasets
        else "n/a",
        "Unstructured biomedical text (NFCorpus)": best_ret("nfcorpus")
        if "nfcorpus" in datasets
        else "n/a",
        "Tables (WikiTableQuestions)": best_ret("wikitablequestions")
        if "wikitablequestions" in datasets
        else "n/a",
        "Charts (ChartQA)": best_ret("chartqa") if "chartqa" in datasets else "n/a",
        "Formulas (FormulaReasoning)": best_ret("formulareasoning")
        if "formulareasoning" in datasets
        else "n/a",
    }
    for k, v in qa.items():
    ...
    for ds in datasets:
        ...
        for m in methods[ds]:
            ...
            if nd is not None and base_nd is not None and nd < base_nd:
                hurt.append(f"{ds}/{m}")
    L.append(
        f"- **Methods that hurt retrieval (nDCG@10 below baseline):** "
    ...
    L.append(
        f"- **Did better retrieval always mean better answers?** No. Cases where "
        f"retrieval improved but answer quality dropped: "
        f"{', '.join(decoupled) if decoupled else 'none observed'}."
    )
    # latency-heaviest method
    lat_heavy = max(
        lat.items(), key=lambda kv: _f(kv[1].get("total_index_time_seconds")) or 0
    )
    L.append(
        f"- **Highest indexing-latency condition:** {lat_heavy[0][0]}/{lat_heavy[0][1]} "
    ...
    for ds in datasets:
        ...
        for m in nb:
            ...
            combo.append(
                (
                    m,
                    (nd - (base_nd or 0))
                    + ((fa - base_fa) if None not in (fa, base_fa) else 0),
                )
            )
        if not combo:
            continue
        ...
        why = (
            "improves retrieval and/or answer grounding"
            if score > 0
            else "no net gain over baseline in this run"
        )
