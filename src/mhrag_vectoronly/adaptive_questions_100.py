...
from mhrag_vectoronly.full_corpus_no_question_baseline import (
    chunk_article,
    cosine_scores,
    locate_fact,
    tokenize,
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    progress.write(
                        f"[generation:error] {chunk['chunk_id']}: {error}"
                    )
                finally:
                    progress.update()
    if failures:
...
def evaluate(
    ...
    for qi, query in enumerate(queries):
        ...
        for name, condition in conditions.items():
            ...
            for ranklist in (dense_rank, sparse_rank):
                for rank, cid in enumerate(ranklist, 1):
                    score[cid] += 1 / (RRF_K + rank)
            ranking = [
                cid for cid, _ in sorted(score.items(), key=lambda x: (-x[1], x[0]))
            ]
