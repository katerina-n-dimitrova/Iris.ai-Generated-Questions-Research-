...
ONLINE_FIELDS = [
    "query_id", "dataset", "condition", "top_k",
    "query_embedding_latency_ms", "chroma_search_latency_ms",
    "context_assembly_latency_ms", "total_retrieval_latency_ms",
    "retrieved_chunk_ids",
]
...
def run_one(client, dataset: str, condition: str, top_k: int, retrieve_k: int,
    ...
    print(f"  {dataset}/{condition}: {len(retrieved_rows)} queries "
          f"-> {out_path.name}")
    return latency_rows
...
def main() -> None:
    ...
    for dataset in args.datasets:
        for condition in args.conditions:
            all_latency += run_one(client, dataset, condition,
