...
def units_scifact(n_queries: int, n_distractors: int, use_llm: bool):
    ...
    if use_llm:
        ...
        ctxs = [
            f"a scientific abstract titled '{d[1]}'"
            if d[1]
            else "a scientific abstract"
            for d in docs
        ]
    ...
    queries = [
        {"query_id": q, "dataset": "scifact", "text": t, "gold_source_ids": g}
        for q, t, g in selected
    ]
    return units, queries


def enrich_scifact_baseline(u):
    return u["text"]


def enrich_scifact_generated_questions(u):
    return _join(
        "Likely questions this chunk answers:",
        u["gen_questions"],
...
def enrich_scifact_doc_summary_position(u):
    ...
    return _join(
        f"Document summary: This is {u['doc_summary']}.",
...
def units_nfcorpus(n_queries: int, n_distractors: int, use_llm: bool):
    ...
    if use_llm:
        ...
        sums = _parallel(
            lambda c: common.llm_summary(
                client, c[2], kind="biomedical passage in plain language"
            ),
            chunks,
        )
    else:
    ...
    if use_llm:
        ds_list = _parallel(
            lambda d: common.llm_summary(
                client,
                str(corpus[d].get("text", ""))[:3000],
                kind="whole biomedical document",
            ),
            doc_ids,
        )
    else:
        ds_list = [
            common.cheap_summary(str(corpus[d].get("text", ""))) for d in doc_ids
        ]
        ...
        units.append(
            {
                ...
                "ci": ci + 1,
                "n_chunks": n_per_doc[doc_id],
                "chunk_key": f"nfcorpus_{doc_id}_c{ci}",
            }
        )
    queries = [
        {"query_id": q, "dataset": "nfcorpus", "text": t, "gold_source_ids": g}
        for q, t, g in selected
    ]
    return units, queries
    ...
    try:
        r = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "List the questions a reader could answer from the passage. "
...
def enrich_nfcorpus_baseline(u):
    return u["text"]


def enrich_nfcorpus_generated_questions(u):
    return _join(
        "Likely questions this passage answers:",
        u["gen_questions"],
...
def enrich_nfcorpus_doc_summary_position(u):
    ...
    return _join(
        f"Document summary: {u['doc_summary']}",
...
def units_wikitablequestions(n_queries: int, n_distractors: int, use_llm: bool):
    ...
    tsums = (
        _parallel(_summarize, tables) if use_llm else [_summarize(t) for t in tables]
    )
...
def enrich_wikitable_baseline(u):
    return u["text"]
...
def enrich_wikitable_combined(u):
    return _join(
        f"Page/table title: {u['title']}",
        f"Columns: {u['headers']}",
        f"Row summary: {u['row_summary']}",
        f"Original row: {u['text']}",
    )
...
# =========================================================================== #
CHART_TABLE_PLACEHOLDER = (
    "[chart-to-table extraction unavailable: requires "
    "OCR/vision/chart-derendering on the chart image]"
)
CHART_AXIS_PLACEHOLDER = (
    "[axis/legend/title metadata unavailable: requires "
    "vision parsing of the chart image]"
)


VISION_PLACEHOLDER = (
    "[vision description unavailable: run with --use-llm to use the "
            ...
            messages=[
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "You are a vision model reading a chart. In 2-4 factual sentences, "
...
def units_chartqa(n_queries: int, n_distractors: int, use_llm: bool):
    ...
    if use_llm:
        summaries = _parallel(
            lambda c: common.llm_summary(client, c[3], kind="chart"), charts
        )
        # vision pass: read the actual chart image with the chat model
        try:
            images = _load_chartqa_images(len(charts))
            visions = _parallel(
                lambda im: _vision_describe(client, im), images, workers=6
            )
        except Exception as e:  # noqa: BLE001
    ...
    units = [
        {
            ...
            "axis_meta": CHART_AXIS_PLACEHOLDER,
            "summary": summ,
            "vision_text": vis,
            "chunk_key": cid,
        }
...
def enrich_chartqa_baseline(u):
    return u["text"]


def enrich_chartqa_data_table(u):
    return _join(
        "Extracted chart data:", u["chart_table"], f"Original chart text: {u['text']}"
    )
...
def enrich_chartqa_summary(u):
    return _join(f"Chart summary: {u['summary']}", f"Original chart text: {u['text']}")


def enrich_chartqa_vision_description(u):
    # chat model used as a vision model: chart description + numeric analysis
    return _join(
        f"Chart (vision-read) description and analysis: {u['vision_text']}",
...
# =========================================================================== #
FORMULA_SURROUNDING_PLACEHOLDER = (
    "[surrounding text unavailable: the formula database stores standalone formulas]"
)
...
def enrich_formula_structure(u):
    return _join(
        f"Formula in LaTeX: {u['latex']}",
        f"Structure: {u['structure']}",
        f"Original: {u['text']}",
    )
...
def enrich_numinamath_baseline(u):
    return u["text"]
...
def enrich_numinamath_problem_context(u):
    return _join(
        f"This step is part of solving the problem: {u['problem'][:400]}",
...
def enrich_numinamath_latex_structure(u):
    return _join(
        f"Formulas/symbols in this step (LaTeX): {u['latex']}" if u["latex"] else "",
        f"Step: {u['text']}",
    )
...
LLM_CONDITIONS = {
    ("scifact", "llm_generated_chunk_context"),
    ("scifact", "doc_summary_position"),
    ("scifact", "generated_questions"),
    ("scifact", "combined_best"),
    ("nfcorpus", "generated_questions"),
    ("nfcorpus", "plain_summary"),
    ("nfcorpus", "doc_summary_position"),
    ("nfcorpus", "combined_best"),
    ("chartqa", "chart_summary"),
    ("chartqa", "vision_chart_description"),
...
def build_dataset(
    dataset: str, n_queries: int, n_distractors: int, use_llm: bool
