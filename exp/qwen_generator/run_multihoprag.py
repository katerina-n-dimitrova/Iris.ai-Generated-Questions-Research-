...
for entry in (ROOT_DIR / "src", ROOT_DIR / "exp" / "multihoprag"):
    if str(entry) not in sys.path:
        sys.path.insert(0, str(entry))

import adaptive_lib as A
import baseline as B
from ragkit.iris_llm_client import IRIS_LLM_MAX_TOKENS, IRIS_LLM_MODEL, stream_iris_chat
...
def configure_chunk_experiment(batch_size: int) -> None:
    ...
    def generate_bounded(chunk: dict) -> dict:
        ...
        facts = A.dedup_facts([item for item in raw_facts if isinstance(item, dict)])
        if not facts:
...
def configure_article_experiment(batch_size: int):
    ...
    import chunk_article_questions as C

    # adaptive_questions (imported by C) changes A's historical cache
...
def run(batch_size: int = 32, skip_missing_articles: bool = False) -> dict:
    ...
    print(
        f"[model] {IRIS_LLM_MODEL}  [inference] streaming_concurrency={batch_size}",
        flush=True,
    )
