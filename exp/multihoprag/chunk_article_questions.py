...
import adaptive_questions as F
from ragkit.fusion import rrf_merge
from ragkit.text import read_jsonl as _read_jsonl
from ragkit.vectors import cosine
...
def generate_articles(articles: list[dict]) -> list[dict]:
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        progress.write(
                            f"[article-generation:error] {article['title']}: {error}"
                        )
                    finally:
                        progress.update()
    if failures:
    ...
    A.write_jsonl(
        ARTICLE_GENERATIONS,
        [
            cached[article["article_id"]]
            for article in articles
            if article["article_id"] in cached
        ],
    )
    return ordered
...
def evaluate(
    ...
    for qi, query in enumerate(queries):
        ...
        ranking = rrf_merge(
            (
                [chunk_ids[i] for i in dense_order],
                [chunk_ids[i] for i in sparse_order],
            ),
            A.RRF_K,
        )
...
def render(payload: dict) -> None:
    ...
    articles_with_questions = combined_quality.get(
        "articles_with_questions", p["articles"]
    )
    articles_without_questions = combined_quality.get("articles_without_questions", 0)
    chunk_note = f"Chunk-level adaptive generation covers all {p['chunks']} chunks."
    article_note = (
